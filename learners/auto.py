...
class AutoGEL(nn.Module):
    def get_flat_grads(self):
        flat_grads = []
        for n,m in self.named_modules():
            if n in self.controller_params:
                temp = m.weight.grad.data.view(-1) / self.args.bs
                flat_grads.append(temp)
        return torch.cat(flat_grads, 0)

    def get_flat_params(self):
        # return torch.cat([p.view(-1) for p in self.model.parameters()], 0)
        # return torch.cat([p.view(-1) for n,p in self.named_parameters() if n in self.controller_params], 0)
        return torch.cat([m.weight.view(-1) for n,m in self.named_modules() if n in self.controller_params], 0)
    ...
    def copy_flat_params(self, cI):
        ...
        for n,m in self.named_modules():
            if n in self.controller_params:
                plen = m.weight.view(-1).size(0)
                m.weight.data.copy_(cI[idx: idx+plen].view_as(m.weight))
                idx += plen

    def transfer_params(self, learner_w_grad, cI):
        # learner_w_grad.copy_flat_params(cI)
        # learner_w_grad.derive_arch()
        ...
        #         temp = cI[idx: idx+plen].view_as(p)
        #         p = cI[idx: idx+plen].view_as(p).clone()
        #         idx += plen
        #     # else:
        #     #     p.detach_()
    ...
    def init_alpha(self):
        self.log_alpha_agg = nn.Linear(self.num_choices['agg'], self.layers, bias=False)
        self.log_alpha_combine = nn.Linear(self.num_choices['combine'], self.layers, bias=False)
        self.log_alpha_act = nn.Linear(self.num_choices['act'], self.layers, bias=False)
        self.log_alpha_layer_connect = nn.Linear(self.num_choices['layer_connect'], self.layers, bias=False)
        self.log_alpha_layer_agg = nn.Linear(self.num_choices['layer_agg'], 1, bias=False)
        self.log_alpha_pool = nn.Linear(self.num_choices['pool'], 1, bias=False)

        # reset parameters for controller
        # default initilization in nn.Linear() is xavier, here we use normal weth mean=1 and std=0.01
        for n,m in self.named_modules():
            if n in self.controller_params:
                nn.init.normal_(m.weight, mean=1, std=0.01)
                # print(m.weight)

        # # self.log_alpha_agg = self.log_alpha_agg_l.weight
        # # self.log_alpha_combine = self.log_alpha_combine_l.weight
        # # self.log_alpha_act = self.log_alpha_act_l.weight
        # # self.log_alpha_layer_connect = self.log_alpha_layer_connect_l.weight
        # # self.log_alpha_layer_agg = self.log_alpha_layer_agg_.weight
        # # self.log_alpha_pool = self.log_alpha_pool_l.weight

        # self.log_alpha_agg = self.log_alpha_agg_l._parameters['weight'].clone()
        # self.log_alpha_combine = self.log_alpha_combine_l._parameters['weight'].clone()
        # self.log_alpha_act = self.log_alpha_act_l._parameters['weight'].clone()
        # self.log_alpha_layer_connect = self.log_alpha_layer_connect_l._parameters['weight'].clone()
        # self.log_alpha_layer_agg = self.log_alpha_layer_agg_l._parameters['weight'].clone()
        # self.log_alpha_pool = self.log_alpha_pool_l._parameters['weight'].clone()

        # self.log_alpha_agg = nn.Parameter(torch.zeros((self.layers, self.num_choices['agg']), device=self.device).normal_(mean=1, std=0.01).requires_grad_())
        # self.log_alpha_combine = nn.Parameter(torch.zeros((self.layers, self.num_choices['combine']), device=self.device).normal_(mean=1, std=0.01).requires_grad_())
        # self.log_alpha_act = nn.Parameter(torch.zeros((self.layers, self.num_choices['act']), device=self.device).normal_(mean=1, std=0.01).requires_grad_())
        # self.log_alpha_layer_connect = nn.Parameter(torch.zeros(self.layers, self.num_choices['layer_connect'], device=self.device).normal_(mean=1, std=0.01).requires_grad_())
        # self.log_alpha_layer_agg = nn.Parameter(torch.zeros(1, self.num_choices['layer_agg'], device=self.device).normal_(mean=1, std=0.01).requires_grad_())
        # self.log_alpha_pool = nn.Parameter(torch.zeros(1, self.num_choices['pool'], device=self.device).normal_(mean=1, std=0.01).requires_grad_())


    def update_z_hard(self, step=0):
        ...
        self.Z_agg_hard = self.get_Z_hard(self.log_alpha_agg.weight, self.temperature, self.args)
        self.Z_combine_hard = self.get_Z_hard(self.log_alpha_combine.weight, self.temperature, self.args)
        self.Z_act_hard = self.get_Z_hard(self.log_alpha_act.weight, self.temperature, self.args)
        self.Z_layer_connect_hard = self.get_Z_hard(self.log_alpha_layer_connect.weight, self.temperature, self.args)
        self.Z_layer_agg_hard = self.get_Z_hard(self.log_alpha_layer_agg.weight, self.temperature, self.args)
        self.Z_pool_hard = self.get_Z_hard(self.log_alpha_pool.weight, self.temperature, self.args)

        self.Z_hard_dict['agg'].append(self.Z_agg_hard.cpu().detach().tolist())
        self.Z_hard_dict['combine'].append(self.Z_combine_hard.cpu().detach().tolist())
        self.Z_hard_dict['act'].append(self.Z_act_hard.cpu().detach().tolist())
        self.Z_hard_dict['layer_connect'].append(self.Z_layer_connect_hard.cpu().detach().tolist())
        self.Z_hard_dict['layer_agg'].append(self.Z_layer_agg_hard.cpu().detach().tolist())
        self.Z_hard_dict['pool'].append(self.Z_pool_hard.cpu().detach().tolist())
        self.Z_hard_dict = dict(self.Z_hard_dict)

        # if step > 10 and step % 5 == 0 and self.temperature >= 1e-20:
        try:
            if step > int(0.1*self.args.epoch) and step % (int(0.1*self.args.epoch)) == 0 and self.temperature >= 1e-20:
    ...
    def derive_arch(self):
        for key in self.search_space.keys():
            ...
            # self.searched_arch_z[key] = self.Z_hard_dict[key][-1]
    ...
    # this func has some redundant (), may refine later
    def rec_load(self, learner):
        self.Z_hard_dict = learner.Z_hard_dict
        self.searched_arch_z = learner.searched_arch_z
        self.searched_arch_op = learner.searched_arch_op
        self.max_step = learner.max_step
        self.best_metric_search = learner.best_metric_search
        self.best_metric_retrain = learner.best_metric_retrain
        self.temperature = learner.temperature
    ...
    def __init__(self, layers, in_features, hidden_features, out_features, prop_depth, args, dropout=0.0):
        ...
        self.controller_params = ['log_alpha_agg', 'log_alpha_combine', 'log_alpha_act',
                                  'log_alpha_layer_connect', 'log_alpha_layer_agg', 'log_alpha_pool']
        self.rec_reset()
    ...
    def short_summary(self):
        return 'Model: {}, #layers: {}, in_features: {}, hidden_features: {}, out_features: {}'.format(self.args.model,
                                                                                                       self.layers,
                                                                                                       self.in_features,
